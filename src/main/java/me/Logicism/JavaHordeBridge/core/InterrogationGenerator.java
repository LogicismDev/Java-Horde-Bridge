...
public class InterrogationGenerator {
    ...
    public JSONObject startInterrogationGeneration(String imageURL) {
        ...
        while (retryCount < 5) {
            try {
                BrowserData generationData = BrowserClient.executePOSTRequestString(new URL(kaiURL + "/interrogate"), new JSONObject().put("url", imageURL).toString(), headers);
    ...
    public JSONObject startStripBackgroundGeneration(String imageURL) {
        ...
        while (retryCount < 5) {
            try {
                BrowserData generationData = BrowserClient.executePOSTRequestString(new URL(kaiURL + "/stripbackground"), new JSONObject().put("url", imageURL).toString(), headers);
